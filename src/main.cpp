#include <glad/glad.h>
#include <GLFW/glfw3.h>
#include <iostream>
#include "Renderer/ShaderProgram.h"

using namespace std;

GLfloat vertex_pos[] = {
	-1, -1, 0,
	0, 1, 0,
	1, -1, 0
};

GLfloat vertex_color[] = {
	0, 1, 1,
	1, 0, 1,
	1, 1, 0
};

const char* vertex_src =
"#version 460\n"
"layout(location = 1) in vec3 vertex_position;"
"layout(location = 0) in vec3 vertex_color;"
"out vec3 color;"
"void main() {"
"   color = vertex_color;"
"   gl_Position = vec4(vertex_position, 1.0);"
"}";

const char* fragment_src =
"#version 460\n"
"in vec3 color;"
"out vec4 f_color;"
"void main(){"
"	f_color = vec4(color, 1);"
"}";


void glfwWindowizeCallback(GLFWwindow* pWindow, int width, int height)
{
	glViewport(0, 0, width, height);
}

void glfwKeyCallback(GLFWwindow* pWindow, int key, int scancode, int act, int mode)
{
	if (key == GLFW_KEY_ESCAPE && act == GLFW_PRESS)
	{
		glfwSetWindowShouldClose(pWindow, GL_TRUE);
	}
}

int main()
{
	if (!glfwInit())
	{
		return -1;
	}

	GLFWwindow* pWindow = glfwCreateWindow(860, 640, "Some game", NULL, NULL);

	if (!pWindow)
	{
		glfwTerminate();
		return -1;
	}

	glfwMakeContextCurrent(pWindow);

	if (!gladLoadGL())
	{
		cout << "Can't load GLAD!" << endl;
	}

	glfwSetWindowSizeCallback(pWindow, glfwWindowizeCallback);
	glfwSetKeyCallback(pWindow, glfwKeyCallback);

	Renderer::ShaderProgram shaderProgram(vertex_src, fragment_src);
	if (!shaderProgram.isCompiled())
	{
	cerr: "Shader Program was not created.\n";
	}

	GLuint vbo_p = 0;
	glGenBuffers(1, &vbo_p);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_p);
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertex_pos), vertex_pos, GL_STATIC_DRAW);

	GLuint vbo_c = 0;
	glGenBuffers(1, &vbo_c);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_c);
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertex_color), vertex_color, GL_STATIC_DRAW);

	GLuint vao = 0;
	glGenVertexArrays(1, &vao);
	glBindVertexArray(vao);

	glEnableVertexAttribArray(0);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_c);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, nullptr);

	glEnableVertexAttribArray(1);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_p);
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, nullptr);

	glClearColor(1, 1, 1, 1);
	
	while (!glfwWindowShouldClose(pWindow))
	{
		glClear(GL_COLOR_BUFFER_BIT);

		shaderProgram.use();
		glBindVertexArray(vao);
		glDrawArrays(GL_TRIANGLES, 0, 3);
		
		glfwSwapBuffers(pWindow);

		glfwPollEvents();
	}

}